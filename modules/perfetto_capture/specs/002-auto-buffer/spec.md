...
自动计算的 `buffer_size_kb` 写入 TraceConfig 的 ring buffer 配置。当前引擎固定使用 **Autobuffer（`--background` + 停止-重启保存）** 模式；Snapshot（`--detach` + `--clone`）代码保留但未启用，因部分设备 clone 产出的 trace 缺少完整 clock snapshot 导致时间戳异常（详见 [001-migration/spec.md](../001-migration/spec.md)）。
...
```
LIGHT_RATE_KB_PER_SEC = 6800       # 轻载档基线 (KB/s)，校准自 7 atrace / 15s ≈ 120 MB 目标
HEAVY_PER_CAT_RATE_KB = 600        # 每多一个 tag 的附加速率 (KB/s)
...
- **轻载档**（`total_tags ≤ 7`）：仅使用基线速率 6800 KB/s。
- **重载档**（`total_tags > 7`）：对超出 7 的每个 tag 追加 600 KB/s。

**校准目标**（游戏场景）：

| 基准场景 | 目标 |
|--------|------|
| **7 个默认 atrace categories、15s、安全系数 1.2** | **约 120 MB** |

**参数选择依据**：
- `LIGHT_RATE_KB_PER_SEC = 6800`：由 120 MB / 15s / 1.2 ≈ 6667 KB/s 向上取整
- `HEAVY_PER_CAT_RATE_KB = 600`：高 category 数场景下每 tag 增量，校准后大幅降低以避免估算过大
- `safety_factor` 默认 **1.2**：在标定速率上预留约 20% 余量以覆盖波动（可在配置中调整）

示例：**7 个 atrace categories**、**0 个 ftrace event**、`duration_sec = 15`、**安全系数 1.2**（轻载档）  
→ `6800 × 15 × 1.2 = 122400` KB ≈ **119.5 MB**
...
| AC-03 | 自动计算值在合理范围 | 15s / 7 atrace categories / 无 ftrace / 默认安全系数 1.2 → 约 **120 MB** 量级 |
