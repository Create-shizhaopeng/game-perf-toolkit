...
- **2026-03-25（帧级）**：若有 @FrameInfo 且可确定最大帧时刻，增加 **`frameinfo_window_chunk`**（最大帧 ± 与秒级相同的 `anomaly_sample_pad_ms`）逐帧 TSV；根目录 **`tests/test_perfdog_insights_service.py`** 覆盖 `PerfdogInsightsService`。
