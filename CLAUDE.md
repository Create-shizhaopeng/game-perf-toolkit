...
7. **日志输出**：MUST 使用统一日志体系（`logging` + `InterceptHandler`），MUST NOT 使用 `print()` 输出诊断/错误/警告信息（CLI `console.print` 交互输出除外）（详见 [.claude/rules/log-panel-rules.md](.claude/rules/log-panel-rules.md)）
