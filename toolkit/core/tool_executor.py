...
from toolkit.core.models import ToolCall, ToolCallStatus, ToolResult
...
class ToolExecutor:
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        tool_def = self._registry.get_entry(tool_call.name)
        if not tool_def:
            return ToolResult(
        ...
        try:
            if inspect.iscoroutinefunction(tool_def.handler):
                raw_result = await tool_def.handler(**tool_call.arguments)
            else:
                raw_result = await asyncio.to_thread(
                    tool_def.handler, **tool_call.arguments
                )
        except Exception as exc:
            logger.error("工具 '%s' 执行异常: %s", tool_call.name, exc, exc_info=True)
