...
class MCPConnection:
    ...
    def __init__(self, server_name="", status=MCPConnectionStatus.DISCONNECTED):
...
class ConnectionPool:
    ...
    async def connect(self, config) -> MCPConnection:
        conn = MCPConnection(
        ...
        try:
            ...
            logger.info("MCP '%s' 已连接，发现 %d 个工具", config.name, len(tools))
            return conn
    ...
    async def _create_session(self, config) -> tuple[Any, list]:
