...
class AgentConfig(BaseModel):
    """Agent 模块配置（LLM 字段已迁移至 llm_manager）。"""
    ...
    max_iterations: int = Field(
        default=20,
        description="单次对话最大工具调用轮次（借鉴 Hermes，默认 20）",
    )
...
def load_config_with_env(path: Path | None = None) -> AgentConfig:
    ...
    # LLM API key resolution has been migrated to llm_manager
    return cfg
...
# Core models — imported from toolkit.core.models
